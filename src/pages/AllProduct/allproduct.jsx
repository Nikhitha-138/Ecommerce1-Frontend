...
const AllProduct = () => {
  ...
  const fetchDetails = async () => {
    try {
      ...
      const newProducts = res.data.filter(item => item.option === 'new');

      setProduct(newProducts);
      setVisible(newProducts);
          ...
          {sortedProducts.length > 0 ? (
            sortedProducts.map(item => (
              ...
              >
                ...
                <h3 className='name'>Stock: {item.stock}</h3>
                <p>⭐ {item.rating}</p>
              </div>
            ))
          ) : (
            <h3 className="noo">No products found under this category</h3>
          )}
        </div>
      </div>
